...
void *test8th2 (param_t *param)
{
  ...
  if ( 40 > (long) param->data ) 
    printf ("\nDequeued:"); 
  for ( i = 0; i < (long) param->data; i ++ ) {
    item = (long) dequeue (param->q, -1);
    if ( (long) (param->data) < 40 )
      printf (" %ld", item ); 
    sum += item; 
  } 
  if ( 40 > (long) param->data )
...
void *test10th2 (param_t *param)
{
  ...
  for ( i = 0; i < (long) param->data; i ++ ) {
    item = (long) dequeue (param->q, -1); /* block */
    sum += item;
  }
...
void *test11th1 (param_t *param)
{
  ...
  for ( i = 0; i < (long) param->data; ++i ) {
    ...
    enqueue (param->q, (void*)item);
    balance += item;
    item = (long) dequeue (param->qa, -1); /* block */
    balance -= item;
  }
...
main (int argc, char *argv[])
{
  ...
  if ( test [test_id] ) {
    ...
    q = createQueue (10);
    if ( ! probe (q) && ! probe (q) ) { /* make sure it releases mutex */
      out ("ok\n");
      total += grade [test_id];
    }else {
      out ("ERROR");
  ...
  if ( test [test_id] ) {
    ...
    q = createQueue (5);
    if ( NULL == head (q) && NULL == head (q) ) { /* make sure it */
      /* releases mutex */
      out ("ok\n");
      total += grade [test_id];
    }else {
  ...
  if ( test [test_id] ) {
    ...
    v2 = &u2;
    enqueue (q, v2);
    if ( v2 == head (q) && v2 == head (q) ) { /*  make sure it */
      /* releases mutex and
	 keeps head in queue */
      out ("ok\n");
      total += grade [test_id];
  ...
  if ( test [test_id] ) {
    ...
    if ( v1 != head (q) && v3 != head (q) && v2 != head (q) ) {
      fprintf (stderr,
	"ERROR head=%#0x v1=%#0x v2=%#0x v3=%#0x v4=%#0x v5=%#0x",
	(unsigned) head (q), (unsigned) v1, (unsigned) v2,
  ...
  if ( test [test_id] ) {
    ...
    pthread_join (th1, &v1);
    pthread_join (th2, &v2);
    fprintf (stderr, "Sum of items: enqueued %ld, dequeued %ld\n",
      (long) v1, (long) v2 );
    pthread_cond_destroy (&cond);
    pthread_mutex_destroy (&me);
    if ( probe (q) || NULL != head (q) ) {
      out ("ERROR queue\n");
