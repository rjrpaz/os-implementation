...
struct Page *FindPageToPageOut()
{
	...
	if ((s_pageList[i].flags & PAGE_PAGEABLE) &&
	    (s_pageList[i].flags & PAGE_ALLOCATED)) {
	    if (!best) best = &s_pageList[i];
	    curr = &s_pageList[i];
...
// Allocate a page of physical memory; assumes that
// interrupts are currently enabled.
void* Alloc_Pageable_Page( pageTableEntry *entry )
{
    void* addr;
    int diskPage;
    struct Page* page;
    ...
    addr = Alloc_Page();
    page = Get_Page( (unsigned int) addr );
    if (!addr) {
	 if (debugFaults) Print("About to hunt for a page to page out\n");
         // need to find a page to kick out and then use it
	 page = FindPageToPageOut();
	 if (debugFaults) Print("selected page at addr %x (age = %d)\n", page->addr, page->clock);

	 addr = (void *) page->addr;

	 // find a place on disk for it
	 diskPage = findSpaceOnPagingFile();

	 // write the page to disk
	 writeToPagingFile(page->addr, diskPage);

	 if (debugFaults) Print("writing physical frame %x to paging file at %d\n", page->addr, diskPage);

	 // update page table to reflect the page being on disk
	 page->entry->present = 0;
	 page->entry->kernelInfo = KINFO_PAGE_ON_DISK;
	 page->entry->pageBaseAddr = diskPage;
